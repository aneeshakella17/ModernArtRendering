...
def blend(img_target, img_source, img_mask):
    ...
    target_size = np.shape(img_target);
    source_size = np.shape(img_source);
    offset = [target_size[0] - source_size[0], target_size[1] - source_size[1]];
    print(offset);
    region_source = (
        max(-offset[0], 0),
        max(-offset[1], 0),
    ...
    print(region_source[0], region_source[1], region_source[2], region_source[3])
...
def test():
    ...
    img_ret = blend(img_target, img_source, img_mask)
